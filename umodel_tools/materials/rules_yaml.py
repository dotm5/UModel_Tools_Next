"""Config-backed texture rule helpers for game profiles (canonical location)."""
...
def _load_sibling_module(module_name: str):
    """Load a .py file from the same directory (e.g., decision.py in materials/)."""
...
def _load_parent_module(module_name: str):
    """Load a .py file from the parent umodel_tools/ directory."""
    module_path = os.path.join(os.path.dirname(__file__), "..", f"{module_name}.py")
...
try:
    ...
    texture_path_utils = _load_parent_module("texture_path_utils")
    decision = _load_sibling_module("decision")
    ConnectionSpec = decision.ConnectionSpec
    MaterialRuleSet = decision.MaterialRuleSet
    NodeSpec = decision.NodeSpec
    TextureRule = decision.TextureRule
...
def _parse_skip_when(raw_skip_when: t.Any, rule_name: str, rule_path: str) -> frozenset[tuple[str, bool]]:
    ...
    for switch_name, expected_value in raw_skip_when.items():
        normalized_name = texture_path_utils.normalize_token(switch_name)
